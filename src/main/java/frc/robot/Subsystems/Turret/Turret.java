...
public class Turret extends SubsystemBase {
  ...
  /** True for any periodic cycle in which the physics-calculated elevation angle
   *  exceeds the mechanical limits and must be clamped. Cleared each periodic. */
  @AutoLogOutput
  private boolean elevationAngleOutOfBounds = false;

  // ─── Shot profile ─────────────────────────────────────────────────────────────
  /**
   * Bundles the three parameters that change with distance so they are always
   * kept consistent and pulled from Constants in one place.
   *
   * ballSpeedMetersPerSec  — used in the ballistic physics calculation
   * flywheelRadiansPerSec  — sent to the flywheel motor (hardware units)
   * useHighArc             — selects the +√ or −√ root of the trajectory formula
   */
  ...
  public enum TurretSystemState {
    TRACKING_TARGET, IDLE, ACTIVE_SHOOTING, ACTIVE_PASSING,
    STOW, ZERO, TESTING, PRACTICE_SHOOTING
  }

  public enum TurretWantedState {
    IDLE, SHOOT_SCORE, PASS_TO_ALLIANCE, PASSIVE_TRACK,
    STOW, ZERO, TESTING, PRACTICE_SHOOTING
  }
  ...
  public void periodic() {
    ...
    predictedTurretPose = new Pose2d(
        turretPose.getX() + (chassisSpeeds.vxMetersPerSecond  * ShooterCalculationConstants.TIME_DELAY),
        turretPose.getY() + (chassisSpeeds.vyMetersPerSecond  * ShooterCalculationConstants.TIME_DELAY),
        turretPose.getRotation().plus(new Rotation2d(
            chassisSpeeds.omegaRadiansPerSecond * ShooterCalculationConstants.TIME_DELAY)));

    currentTurretMeasurables = new TurretMeasurables(
  ...
  public TurretSystemState handleStateTransitions() {
    switch (wantedState) {
      ...
      case SHOOT_SCORE:       return TurretSystemState.ACTIVE_SHOOTING;
      case PASS_TO_ALLIANCE:  return TurretSystemState.ACTIVE_PASSING;
      case PASSIVE_TRACK:     return TurretSystemState.TRACKING_TARGET;
      case TESTING:           return TurretSystemState.TESTING;
      case ZERO:              return TurretSystemState.ZERO;
      case STOW:              return TurretSystemState.STOW;
      case PRACTICE_SHOOTING: return TurretSystemState.PRACTICE_SHOOTING;
  ...
  public void applyStates() {
    switch (systemState) {
      ...
      case ZERO:
        wantedTurretMeasurables = new TurretMeasurables(
  ...
  /**
   * Returns the appropriate ShotProfile for a scoring shot based on distance.
   * All tiers use high arc for top-entry descent angle.
   *
   *  < 2m    : dead zone  — elevationAngleOutOfBounds will always fire
   *  2 – 3m  : CLOSE  7.5 m/s  launch 68°–77°  descent 51°–68°
   *  3 – 4.5m: FAR    8.5 m/s  launch 65°–75°  descent 53°–69°
   *  4.5–6.1m: MID    10  m/s  launch 68°–75°  descent 62°–71°
   *  > 6.1m  : MID profile still returned — command layer should prefer passing
   */
  private ShotProfile selectShotProfile(Pose2d target) {
    double distance = target.getTranslation()
        .getDistance(predictedTurretPose.getTranslation());
   ...
   * Reuses scoring profiles at close range since they are already
   * the cheapest option at those distances.
   *
   *  0  – 3m   : CLOSE        7.5 m/s
   *  3  – 4.5m : FAR          8.5 m/s
   *  4.5– 10.5m: PASSING_SLOW 11  m/s
   *  10.5m+    : PASSING_FAST 12  m/s
   */
  ...
  /**
   * Computes wantedTurretMeasurables (elevation, rotation, flywheel speed) to
   * hit desiredPose at desiredHeight while compensating for robot inertia.
   *
   * Physics note:
   *   V_shooter_robot = V_noInertia_field − V_robot_field
   *   At launch:  V_ball_field = V_shooter_robot + V_robot_field = V_noInertia_field  ✓
   *
   * Unit note:
   *   updateWithCartesianVector() returns a speed magnitude in m/s (ball speed).
   *   After the vector math, wantedTurretMeasurables.shooterRadiansPerSec is
   *   overwritten with the profile's flywheel rad/s so that hardware units and
   *   physics units are never mixed.
   */
  private void performShootingCalc(ShotProfile profile) {
    Rotation2d fieldAngle = findFieldCentricAngleToTarget(desiredPose);
    Rotation2d elevAngle  = calculateElevationAngle(
        desiredPose, profile.ballSpeedMetersPerSec, profile.useHighArc);
    ...
    TurretMeasurables noInertiaMeasurables = new TurretMeasurables(
        elevAngle,
        fieldAngle,
        profile.ballSpeedMetersPerSec);
  ...
  private void goToWantedState() {
    ...
    shooterIO.setVelo(AngularVelocity.ofBaseUnits(
  ...
  private boolean atSetpoint() {
    ...
    Logger.recordOutput("Turret/Setpoint/currentElevationDeg",  currentTurretMeasurables.elevationAngle.getDegrees());
    Logger.recordOutput("Turret/Setpoint/wantedElevationDeg",   wantedTurretMeasurables.elevationAngle.getDegrees());
    Logger.recordOutput("Turret/Setpoint/currentRotationDeg",   currentTurretMeasurables.rotationAngle.getDegrees());
    Logger.recordOutput("Turret/Setpoint/wantedRotationDeg",    wantedTurretMeasurables.rotationAngle.getDegrees());
    Logger.recordOutput("Turret/Setpoint/currentShooterRadPs",  currentTurretMeasurables.shooterRadiansPerSec);
    Logger.recordOutput("Turret/Setpoint/wantedShooterRadPs",   wantedTurretMeasurables.shooterRadiansPerSec);
    Logger.recordOutput("Turret/Setpoint/shooterTolerance",     shooterTolerance);

    boolean elevationAtSetpoint = MathUtil.isNear(
    ...
    Logger.recordOutput("Turret/Setpoint/atSetpoint",          elevationAtSetpoint && rotationAtSetpoint && shooterAtSetpoint);

    return elevationAtSetpoint && rotationAtSetpoint && shooterAtSetpoint;
  }

  /**
   * Returns how far above the physical rim to aim, based on distance.
   * Aiming above the rim ensures the ball is already descending when it
   * reaches the rim level, improving top-entry angle consistency.
   * At longer range where descent angle is shallower we aim higher to
   * compensate — but speed limits mean this has diminishing returns beyond 4m.
   */
   ...
   *
   * Formula derivation (range equation solved for θ):
   *   tanθ = (v² ± √(v⁴ − g(gx² + 2yv²))) / (gx)
   *   +√ → high arc (more hang time, steeper angle)
   *   −√ → low arc  (flatter, faster to target)
   *
   ...
   *
   * Step 1: θ_rel_ccw = θ_field − θ_predictedRobotHeading  (still CCW+)
   * Step 2: θ_turret  = −θ_rel_ccw                         (flip to CW+)
   *
   * Uses predictedTurretPose heading — not current odometry — because the
   * field-centric angle was computed against that future heading.
   */
  ...
  /**
   * Maps the unbounded robot-relative angle onto [MIN_ANGLE, MAX_ANGLE] by
   * choosing the shortest path from the current unwrapped encoder position,
   * then clamps elevation within its mechanical limits.
   */
  public void convertToBoundedTurretAngle() {
    ...
    if (Math.abs(
        ...
        < ANGLE_EPSILON) {
      finalOffset = (Math.abs(positivePath) <= Math.abs(negativePath))
    ...
    double clampedElevation = MathUtil.clamp(
        rawElevation,
